...
public class GameOfLife : MonoBehaviour
{
	...
	private void Start()
	{
		...
		lifeGrid = new GameObject[width, height];
	...
	private void NextGeneration(ref bool[,] grid)
	{
		...
		for (int row = 0; row < grid.GetLength(0); row++)
		{
			for (int col = 0; col < grid.GetLength(1); col++)
			{
				...
				{
					lifeGrid[row, col].GetComponent<Renderer>().material.color = nextGrid[row, col] ? Color.black : Color.white;
				}
			}
		}
		grid = nextGrid;
